...
function Config({tip,inapoi}){
  try {
    // Verifică dacă tipul există
    const T = C.tipuri[tip];
    if (!T) {
      return <div style={{padding:40, background:"#fff3f0", border:"2px solid #e74c3c", margin:20, borderRadius:8}}>
        <h2 style={{color:"#c0392b"}}>❌ Eroare: tipul "{tip}" nu există în configurație</h2>
        <p style={{fontSize:14}}>Te rugăm să selectezi un alt tip sau să verifici CONFIG.js</p>
        <button onClick={inapoi} style={{padding:"8px 16px",background:"#a37e4a",color:"#fff",border:"none",borderRadius:6,cursor:"pointer"}}>Înapoi</button>
      </div>;
    }
            ...
            return (
              <div key={cat} style={{borderBottom:"1px solid #f0ece5"}}>
                ...
                {open && <div style={{paddingBottom:8}}>
                  {Object.entries(items).map(([nume,a]) => (
    ...
    return <div style={{padding:40, background:"#fff3f0", border:"2px solid #e74c3c", margin:20, borderRadius:8}}>
      ...
      <p><strong>Mesaj:</strong> {err.message}</p>
